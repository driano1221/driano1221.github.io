...
export function StoryLearning({ locale }: { locale:'pt'|'en' }) {
  ...
  return <section id="learning" className="story-section" aria-labelledby="learn-title">
    <div className="story-prose">
      ...
      <h2 id="learn-title">{t('Mesma forma. Outra maneira de aprender.','Same shape. A different way to learn.')}</h2>
      <p>{t('Vamos sair da conta pronta. Agora temos flores de duas espécies e duas medidas de cada pétala: comprimento e largura. Queremos uma regra que separe uma espécie da outra.','Let’s leave the ready-made calculation behind. Now we have flowers from two species and two measurements of each petal: length and width. We want a rule that separates one species from the other.')}</p>
      <p>{t('Cada ponto abaixo é uma flor real do conjunto Iris. A linha é uma decisão: de um lado, setosa; do outro, versicolor. Fisher e um perceptron podem desenhar esse mesmo tipo de linha. A diferença está em como a encontram.','Each point below is a real flower from the Iris dataset. The line is a decision: setosa on one side, versicolor on the other. Fisher and a perceptron can draw this same kind of line. What differs is how they find it.')}</p>
    </div>
    <div className="story-lab learning-lab">
      ...
      <div className="learning-layout">
        ...
        <div className="learning-explanation" aria-live="polite">
          ...
          <p>{method==='fisher'?t('Fisher compara os centros das espécies e a dispersão das medidas dentro de cada grupo. Dessa comparação sai a direção usada para separá-las. Os sinais + marcam os centros, não flores extras.','Fisher compares the species’ centers and how measurements vary within each group. That comparison produces a direction to separate them. The + signs mark centers, not extra flowers.'):t('O perceptron começa com pesos iguais a zero. Quando erra uma flor — ou fica exatamente no limite — ajusta os pesos. Avance pelas correções para ver a decisão mudar.','The perceptron starts with zero weights. When it gets a flower wrong — or lands exactly on the boundary — it adjusts the weights. Step through the corrections to see the decision change.')}</p>
