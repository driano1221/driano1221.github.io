...
export function KindleOcean({ locale }: { locale: Locale }) {
  ...
  return <div className="k-home" data-ktheme={theme}>
    ...
    <main className="k-main k-article" id="post">
      ...
      <p>{t(
        'Para testar, rodei o mesmo pedido em dois agentes, o Claude Code com Opus 5.5 e o ZCode com GLM 5.3 flash. Os dois chegaram aos mesmos números e seguiram boa parte das regras, e duas ideias do ZCode entraram na versão final: a linha tracejada com a média de 1991 a 2020 e mais cuidado ao falar de causa. Mesmo com toda essa ajuda, ainda refinei o código e a estética na mão até o resultado ficar mais bonito e com a minha assinatura. A mesma série também virou uma animação curta, pensada para redes sociais.',
        'To test it, I ran the same request in two agents, Claude Code with Opus 5.5 and ZCode with GLM 5.3 flash. Both reached the same numbers and followed most of the rules, and two of ZCode's ideas made it into the final version: the dashed line with the 1991 to 2020 average and more care when talking about causes. Even with all that help, I still refined the code and the aesthetics by hand until the result looked sharper and carried my signature. The same series also became a short animation, meant for social media.',
      )}</p>
      <figure>
