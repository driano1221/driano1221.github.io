...
export function damage(count=8) {
  ...
  if (count > damagedIndexes.length) {
    // Ruído extra espalhado pelo fundo: preserva o traço do padrão
    // (flips sequenciais apagavam o desenho antes de recuperá-lo).
    memory.forEach((_, i) => {
      if (flipped.size < count && !flipped.has(i) && memory[i] < 0 && i % 3 === 0) flipped.add(i);
    });
  }
